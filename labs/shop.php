...
<body>
    <div class="container">
        ...
        <form action="shop_add.php" method="post" encodetype="multipart/form-data">
            <input type="text" name="id" value="<?=$row["id"] ?? ''?>" hidden>
            <input type="hidden" name="x" value="<?=$x?>">
            <input type="hidden" name="y" value="<?=$y?>">
            <input type="text" name="old_img" value="<?=$row['shop_img'] ?? ''?>" hidden>
            ...
            <div class="form-warpper">
                ...
                <select name="shop_category" id="shop_category">
                    <option value="อาหาร"      <?=($row["shop_category"] ?? '')=="อาหาร"?'selected':''?>>อาหาร</option>
                    <option value="เสื้อผ้า"   <?=($row["shop_category"] ?? '')=="เสื้อผ้า"?'selected':''?>>เสื้อผ้า</option>
                    <option value="เครื่องดื่ม" <?=($row["shop_category"] ?? '')=="เครื่องดื่ม"?'selected':''?>>เครื่องดื่ม</option>
                </select>
            </div>

            <div class="form-warpper">
                <label for="shop_description">รายละเอียดร้านค้า</label>
                <input type="text" name="shop_img" id="shop_img"
                       value="<?=$row["shop_description"] ?? ''?>">
            </div>

            <div class="form-warpper">
                <label for="shop_description">รูปภาพ</label>
                <input type="file" name="shop_description" id="shop_description"
                       value="<?=$row["shop_description"] ?? ''?>" accept="image/*">
            </div>
